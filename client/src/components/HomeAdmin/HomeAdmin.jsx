  ...
  return (
    <div className="h-screen w-screen bg-primary flex flex-col justify-center items-center gap-10 mb-52">
      <h1 className="mt text-4xl font-bold text-tertiary mt-20">
        <span className="text-secundary">B</span>ienvenido,{" "}
        <span className="text-secundary">A</span>dmin
      </h1>
        ...
        {allContact.map((item) => {
          const filteredConsultas = allContact.filter((consulta) => {
            if (item.name === "Consultas de propiedades") {
              return (
                consulta.tipoConsulta === "inmuebles" &&
            ...
            } else if (item.name === "Consultas de propiedades atendidas") {
              return (
                consulta.tipoConsulta === "inmuebles" &&
                consulta.state === "completo"
              );
            } else if (item.name === "Otras consultas atendidas") {
              return (
                consulta.tipoConsulta !== "inmuebles" &&
                consulta.state === "completo"
              );
          ...
          return (
            ...
            >
              <button
                className={`w-full h-full hover:scale-110 cursor-pointer bg-red flex justify-center items-center ${
                  showCards[item.id] ? "hidden" : "flex"
                }`}
              ...
              <div
                className={`bg-secundary h-full w-full flex flex-col overflow-y-auto items-center transition-all ${
                  showCards[item.id]
                    ? "flex card-enter scroll-invisible"
                    : "card-exit"
                }`}
              >
                <button
                ...
                {filteredConsultas.map((consulta) => (
                  <Card
                    key={consulta._id}
                    onClick={() => handleClick(consulta._id)}
